                    ...
                    <el-popover
                        ...
                        <div>
                            <div class="avatar_userInfo">
                                ...
                                <ul>
                                    <li>帐号经理：<span>{{ serviceInfoList.service_name }}</span></li>
                                    <li>交付订单：<span>{{ serviceInfoList.pay_order }}</span></li>
                                    <li>客户评价：<el-rate v-model="serviceInfoList.customer_reviews" disabled :colors="['#796CF3','#796CF3','#796CF3']"></el-rate></li>
                                    <li>客户投诉：<span>{{ serviceInfoList.customer_report }}</span></li>
                                </ul>
                            </div>
        ...
        handleCheckEnterpriseQr(){
            ...
            _this.checkEnterpriseQr = setInterval(()=>{
                checkEnterpriseQr({
                    ...
                    .then((res) => {
                        if(res.code === 1 && res.data.static === 0){//请等待扫码
                            timer++;
                            _this.dialog = true;
...
<style lang="less">
.menu_popover{
    background: linear-gradient(180deg, #F2F0FE 0%, #FFFFFF 43%, #FFFFFF 100%) !important;
