    ...
    <div class="content">
      <el-row :gutter="30" style="max-width: 1200px; margin: auto">
        <el-col :span="9" style="padding-left: 0">
          <div class="homepage_header">
            ...
            <div class="title">
              <span
                v-for="(item, index) in userInfo.category_ids"
                :style="{
                  color:
                    index % 3 == 0
                      ? '#4BB1F1'
        ...
        <el-col :span="15" style="padding-right: 0">
          <el-card
            class="box_card basic_information_card"
            style="margin-bottom: 20px"
          >
            <h4>基本资料</h4>
            <div>
              年龄：<span style="margin-right: 40px">{{ userInfo.age }}岁</span
              >性别：<span>{{
                userInfo.genderdata === "male" ? "男" : "女"
              }}</span>
            </div>
            <div>
          ...
          <el-card class="box_card basic_information_card">
            ...
            <el-row style="padding-bottom: 0">
              <el-col :span="12" style="padding-bottom: 0">
                <div>
                  卖点呈现：<span>{{ userInfo.sellingpoint_id }}</span>
                </div>
                <div>
                  视频上传：<span>{{ userInfo.videoupload_id }}</span>
                </div>
              </el-col>
              <el-col :span="12" style="padding-bottom: 0">
                <div>
                  拍摄场景：<span>{{ userInfo.scene_id }}</span>
                </div>
                <div>
                  交付周期：<span>{{ userInfo.leadtime_id }}</span
                  ><b>（样品收货后）</b>
                </div>
              </el-col>
            </el-row>
            <div>
              视频时长：<span
                >通常{{ userInfo.minvideo }}-{{ userInfo.maxvideo }}s</span
              >
            </div>
            <div>
              其他说明
              <p :title="userInfo.content">{{ userInfo.content }}</p>
            </div>
          </el-card>
        </el-col>
      </el-row>
      ...
      <div class="product_box">
        <div style="max-width: 1200px; margin: auto">
          <div
            class="product_item"
            v-for="(item, index) in userInfo.videos"
            :key="index"
          >
...
export default {
  ...
  methods: {
    ...
    handlePlayVideo(data) {
      ...
      let src = data.file;
      $(".videoView").html(
        '<video id="my-video" ref="videoPlayerRef" class="video-js" controls preload="auto" width="640" height="264" poster="' +
          poster +
      ...
      influencerDetail({
        ...
        .then((res) => {
          if (res.code === 1) {
            ...
            this.countryName = res.data.country_id.name;
          }
        })
        .catch((err) => {
          console.log(err);
...
#homepage {
  ...
  .content {
    ...
    .box_card {
      background: #ffffff
        linear-gradient(199deg, rgba(214, 216, 255, 0.36) 0%, #ffffff 100%);
      box-shadow: 0px 10px 20px 0px rgba(0, 0, 0, 0.05);
      border-radius: 20px;
      padding: 0 10px;
    }
